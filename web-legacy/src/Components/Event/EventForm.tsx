...
import { createDBEntry, deleteDBEntry, getDBEntry, updateDBEntry } from 'util/api'
...
const EventForm: React.FC<NewEventProps> = ({ eventData, cancelHandler }) => {
  ...
  useEffect(() => {
    ...
    runThis()
  }, [location.pathname, isAuthenticated, user, params.id, eventData?.trip._id])
  ...
  const inputOnChange = ({
    ...
    setFormInputs({
      ...
      [type]: {
        ...
        val: value,
        valid:
          (type === 'title' || type === 'location' || type === 'datetime' || type === 'category') &&
          value === ''
            ? false
            : true,
      },
    ...
    if (type === 'trip') {
      setTripId(userTrips?.filter((trip) => trip.title === value).map((trip) => trip._id)[0])
    }
  }
    ...
    <FlexboxCol>
      ...
      <FormContents method="post">
        ...
        <FlexboxRow style={{ marginBottom: '10px' }}>
          ...
          {userTrips &&
            (userTrips?.length === 1 ? (
              <input type="text" name="trip" id="trip" value={userTrips[0].title} readOnly />
            ) : (
              <div>
                <input
                  list="trips"
                ...
                />
                <datalist id="trips">
                  {userTrips?.map((trip) => <option key={trip._id} value={trip.title} />)}
                </datalist>
              </div>
            ))}
          {/* {userTrips?.length === 1 ? (
        ...
        {location.pathname.includes('events') ? (
          <FlexboxRow>
            <FormSubmitButton type="button" onClick={(event) => submitEventForm(event, 'delete')}>
              Delete
            </FormSubmitButton>
            <FormSubmitButton type="button" onClick={(event) => submitEventForm(event, 'update')}>
              Save
            </FormSubmitButton>
          </FlexboxRow>
        ) : (
          <FormSubmitButton type="button" onClick={(event) => submitEventForm(event, 'create')}>
            Create
          </FormSubmitButton>
        )}
      </FormContents>
